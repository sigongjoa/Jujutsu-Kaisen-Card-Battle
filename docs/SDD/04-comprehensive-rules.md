...
### 5.2 무한 루프 방지 (Infinite Loop Prevention)
...
규칙 (Frequency Counting):
1. "동일한 카드(InstanceID)의 동일한 능력(AbilityID)"은 한 턴에 최대 10회까지만 발동 가능합니다.
2. 10회 도달 시, 해당 능력은 그 턴 동안 "침묵(Silenced)" 상태가 되어 더 이상 발동하지 않습니다.
3. 이는 강제 종료가 아닌, "과부하로 인한 기능 정지"로 해석됩니다.

예외:
- 플레이어가 의도적으로 루프를 형성하는 경우에도 이 규칙은 적용됩니다.
- "무한 콤보" 덱은 존재할 수 있지만, 한 턴에 낼 수 있는 데미지/효과는 제한됩니다.
